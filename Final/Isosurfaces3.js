function Isosurfaces( volume, isovalue,color,screen,flag )
{
    //flag = 0;
...
//	var B = 1.0-S;
	...
	var G = Math.max( Math.cos( ( S - 0.5 ) * Math.PI ), 0.0 );
        ...
        var color2 = new THREE.Color( R, G, B );
        cmap.push( [ S, '0x' + color2.getHexString() ] );
    }
	...
	var lines = volume.resolution.x;
        var slices = volume.resolution.x * volume.resolution.y;
	var vh=new THREE.Vector3(v0.x*volume.values[v0.x][0],v0.y*volume.values[v0.y][0],v0.z*volume.values[v0.z][0]);
	var vi=new THREE.Vector3(v1.x*volume.values[v1.x][0],v1.y*volume.values[v1.y][0],v1.z*volume.values[v1.z][0]);
	
	var vx = volume.values[v0.x+v0.y*lines+v0.z*slices][0]-s;
	if(vx<0){vx=-vx;}
	var vx2 = volume.values[v1.x+v1.y*lines+v1.z*slices][0]-s;
	if(vx2<0){vx2=-vx2;}

	var vnew0 = v0.multiplyScalar(vx2);
	var vnew1 = v1.multiplyScalar(vx);
	var vs = vx+vx2;
        return new THREE.Vector3().addVectors( vnew0, vnew1 ).divideScalar( vs );
    }
}
